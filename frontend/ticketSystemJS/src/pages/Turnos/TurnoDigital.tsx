...
type Paso = "tipo" | "datos" | "generando";
...
function TurnoDigital() {
  ...
  const formatearDNI = (valor: string) => {
    ...
    if (soloNumeros.length <= 8) return `${soloNumeros.slice(0, 4)}-${soloNumeros.slice(4)}`;
    return `${soloNumeros.slice(0, 4)}-${soloNumeros.slice(4, 8)}-${soloNumeros.slice(8)}`;
  };

  const capitalizar = (texto: string) =>
    texto.trim().toLowerCase().split(" ").filter(Boolean)
      .map((p) => p.charAt(0).toUpperCase() + p.slice(1)).join(" ");

  const construirNombreCompleto = () =>
    [primerNombre, segundoNombre, primerApellido, segundoApellido]
      .map(capitalizar).filter(Boolean).join(" ");
  ...
  const handleGenerar = async (nombreConfirmado?: string) => {
    ...
    if (!nombreFinal.trim()) { setError("Debe ingresar nombre completo."); return; }
    ...
    try {
      const res = await fetch(`${API_URL}/tickets/manual`, {
        ...
        body: JSON.stringify({
          nombreCompleto: nombreFinal, dni, tipoTurno: tipoSeleccionado,
          kioskoId: Number(kioskoId), sucursalId: Number(kioskoSucursalId),
          origenTicket: "DIGITAL",
        }),
      });
      const data = await res.json();
      if (!res.ok) { setError(data.message || "Error al crear el ticket."); setPaso("datos"); return; }
      await new Promise((r) => setTimeout(r, 1200));
      navigate("/ticket-digital-generado", {
        replace: true,
        state: { ticket: data, kioskoId, kioskoNumero, sucursalId: kioskoSucursalId, tipoTurno: tipoSeleccionado, nombreCompleto: nombreFinal, dni },
      });
    } catch {
      ...
      <div className="tm-root">
        ...
        <div className="tm-body">
          ...
          <div className="tm-hero">
            <div className="tm-hero-eyebrow">Sistema de Turnos</div>
            <h1>Genera tu turno <em>digital</em></h1>
            <p>Completa los pasos desde tu celular para generar tu ticket</p>
          </div>
            ...
            <div className="tm-card">
              <div className="tm-card-head">
                ...
                <div>
                  ...
                  <div className="tm-card-sub">Selecciona el tipo de turno</div>
                </div>
              </div>
            ...
            <div className="tm-card">
              ...
              <div className="tm-card-foot">
                <button
                  ...
                  disabled={dni.length !== 15 || (!clienteEncontrado && !mostrarInputsNombre)}
                >
